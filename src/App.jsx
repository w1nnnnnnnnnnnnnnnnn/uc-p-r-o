...
function ClassicDeck({ armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <Tonearm id={id} pivotX={471} pivotY={119} armAngle={armAngle} armLen={armLen} accent="#111" />
...
function DarkDeck({ armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <rect x="452" y="34" width="74" height="492" rx="10" fill="rgba(0,0,0,0.44)" stroke="rgba(255,255,255,0.1)" />
      ...
      {[[489, 138, "GAIN"], [489, 212, "PITCH"], [489, 286, "EQ"]].map(([x, y, lbl]) => (
        <g key={lbl}>
          <circle cx={x} cy={y} r="18" fill={`url(#${id}-knob)`} stroke="rgba(255,255,255,0.16)" />
          <line x1={x} y1={y - 7} x2={x} y2={y - 14} stroke="#ff3030" strokeWidth="2.5" strokeLinecap="round" />
          <text x={x} y={y + 30} fill="rgba(255,255,255,0.38)" fontSize="7" fontFamily="monospace" textAnchor="middle">{lbl}</text>
        </g>
      ))}

      <rect x="468" y="348" width="42" height="94" rx="14" fill="rgba(255,255,255,0.08)" />
      <rect x="462" y="386" width="54" height="22" rx="11" fill="rgba(255,255,255,0.42)" />
      ...
      <circle cx={pivotX} cy={pivotY} r="22" fill="rgba(255,255,255,0.1)" stroke="rgba(255,255,255,0.18)" />
      <circle cx={pivotX} cy={pivotY} r="11" fill={`url(#${id}-knob)`} />
      <Tonearm id={id} pivotX={pivotX} pivotY={pivotY} armAngle={armAngle} armLen={armLen} accent="#111" />
...
function ChromeDeck({ armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <rect x="452" y="72" width="76" height="394" rx="14" fill="rgba(255,255,255,0.22)" stroke="rgba(0,212,255,0.38)" />
      ...
      {[[490, 206, "TRIM"], [490, 270, "GAIN"], [490, 334, "EQ"]].map(([x, y, lbl]) => (
        <g key={lbl}>
          <circle cx={x} cy={y} r="17" fill={`url(#${id}-knob)`} stroke="rgba(0,212,255,0.45)" />
          <line x1={x} y1={y - 7} x2={x} y2={y - 14} stroke="#00d4ff" strokeWidth="2.5" strokeLinecap="round" />
          <text x={x} y={y + 30} fill="rgba(0,0,0,0.48)" fontSize="7" fontFamily="monospace" textAnchor="middle">{lbl}</text>
        </g>
      ))}

      <rect x="466" y="388" width="48" height="52" rx="14" fill="rgba(0,0,0,0.22)" stroke="rgba(255,255,255,0.24)" />
      <circle cx="490" cy="414" r="13" fill="rgba(0,212,255,0.72)" filter={`url(#${id}-glow)`} />
      ...
      <circle cx={pivotX} cy={pivotY} r="23" fill="rgba(255,255,255,0.32)" stroke="rgba(0,212,255,0.35)" />
      <circle cx={pivotX} cy={pivotY} r="11" fill={`url(#${id}-knob)`} />
      <Tonearm id={id} pivotX={pivotX} pivotY={pivotY} armAngle={armAngle} armLen={armLen} accent="#00d4ff" />
...
function WoodDeck({ armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <rect x="452" y="42" width="74" height="472" rx="14" fill="rgba(0,0,0,0.25)" stroke="url(#wood-brass)" strokeWidth="1.5" />

      <rect x="462" y="58" width="54" height="22" rx="8" fill="url(#wood-brass)" opacity="0.88" />
      <text x="489" y="73" fill="#4a3000" fontSize="7" fontFamily="serif" textAnchor="middle">HI-FI</text>

      {[[489, 118, "VOL"], [489, 194, "BASS"], [489, 270, "TREB"], [489, 346, "BAL"]].map(([x, y, lbl]) => (
        <g key={lbl}>
          <circle cx={x} cy={y} r="18" fill="url(#wood-knob)" stroke="rgba(0,0,0,0.35)" />
          <line x1={x} y1={y - 7} x2={x} y2={y - 13} stroke="#4a2e08" strokeWidth="2.2" strokeLinecap="round" />
          <text x={x} y={y + 30} fill="#f0ca65" fontSize="7" fontFamily="monospace" textAnchor="middle">{lbl}</text>
        </g>
      ))}

      <rect x="462" y="408" width="54" height="56" rx="10" fill="rgba(0,0,0,0.26)" stroke="url(#wood-brass)" />
      <text x="489" y="426" fill="#f0ca65" fontSize="7" fontFamily="monospace" textAnchor="middle">RPM</text>
      <circle cx="478" cy="444" r="7" fill="#1a0e04" stroke="url(#wood-brass)" />
      <circle cx="500" cy="444" r="7" fill="#1a0e04" stroke="url(#wood-brass)" />
      ...
      <circle cx={pivotX} cy={pivotY} r="23" fill="#2b1808" stroke="url(#wood-brass)" strokeWidth="2" />
      <circle cx={pivotX} cy={pivotY} r="12" fill="url(#wood-knob)" />

      {(() => {
        const nx = pivotX - armLen;
        return (
          <g transform={`rotate(${armAngle} ${pivotX} ${pivotY})`}>
            <rect x={nx} y={pivotY - 4} width={armLen} height="8" rx="4" fill="url(#wood-arm)" />
            <rect x={nx + 2} y={pivotY - 3.5} width={armLen - 4} height="3" rx="1.5" fill="rgba(255,255,255,0.3)" />
            <rect x={nx - 14} y={pivotY - 9} width="22" height="18" rx="3" fill="#c8952a" stroke="#7a5018" strokeWidth="0.8" />
            <rect x={nx - 12} y={pivotY + 2} width="16" height="8" rx="2" fill="#1a0e04" />
            <line x1={nx - 5} y1={pivotY + 10} x2={nx - 5} y2={pivotY} stroke="#5a360b" strokeWidth="1.5" />
            <ellipse cx={pivotX + 18} cy={pivotY} rx="12" ry="8" fill="url(#wood-brass)" stroke="#7a5018" />
          </g>
        );
      })()}
...
function RealisticDeck({ variant, color, armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <rect x="452" y="142" width="74" height="312" rx="10" fill="rgba(0,0,0,0.24)" stroke="rgba(255,255,255,0.18)" />
      <rect x="466" y="162" width="18" height="46" rx="6" fill={darken(c, 8)} stroke="#555" />
      <rect x="469" y="180" width="12" height="11" rx="3" fill="#aaa" />

      <circle cx="494" cy="204" r="12" fill="url(#r1-knob)" stroke="#666" />
      <line x1="494" y1="194" x2="494" y2="200" stroke="#ccc" strokeWidth="1.5" strokeLinecap="round" />

      <rect x="472" y="232" width="8" height="60" rx="4" fill={darken(c, 22)} stroke="#444" />
      <rect x="467" y="250" width="18" height="13" rx="4" fill="#c8c8c8" filter="url(#r1-soft)" />

      {[["45", 314], ["33", 350]].map(([lbl, y]) => (
        <g key={lbl}>
          <line x1="462" y1={y} x2="490" y2={y} stroke="#777" strokeWidth="0.8" />
          <text x="496" y={y + 4} fill="#999" fontSize="9.5" fontFamily="monospace">{lbl}</text>
        </g>
      ))}

      <rect x="462" y="374" width="54" height="22" rx="8" fill={darken(c, 4)} stroke="#555" />
      <text x="489" y="389" fill="#aaa" fontSize="8" fontFamily="monospace" textAnchor="middle">START</text>
      <rect x="462" y="404" width="54" height="22" rx="8" fill={darken(c, 4)} stroke="#555" />
      <text x="489" y="419" fill="#aaa" fontSize="8" fontFamily="monospace" textAnchor="middle">STOP</text>

      <circle cx={pivotX} cy={pivotY} r="24" fill={mid} stroke={lighten(c, 35)} strokeWidth="1.5" filter="url(#r1-soft)" />
      <circle cx={pivotX} cy={pivotY} r="12" fill="url(#r1-arm)" />
      <circle cx={pivotX} cy={pivotY} r="5" fill="#e0e0e0" />

      {(() => {
        const nx = pivotX - armLen;
        return (
          <g transform={`rotate(${armAngle} ${pivotX} ${pivotY})`}>
            <rect x={nx} y={pivotY - 4.5} width={armLen} height="9" rx="4.5" fill="url(#r1-arm)" />
            <rect x={nx + 2} y={pivotY - 4} width={armLen - 4} height="3.5" rx="1.5" fill="#e0e0e0" opacity="0.35" />
            <rect x={nx - 13} y={pivotY - 10} width="24" height="20" rx="3" fill="#b8b8b8" stroke="#888" />
            <rect x={nx - 10} y={pivotY + 1} width="16" height="10" rx="2" fill="#444" stroke="#666" />
            <line x1={nx} y1={pivotY + 11} x2={nx} y2={pivotY} stroke="#222" strokeWidth="2" strokeLinecap="round" />
            <circle cx={nx} cy={pivotY} r="2.5" fill="#111" />
            <ellipse cx={pivotX + 18} cy={pivotY} rx="13" ry="9" fill="#aaa" stroke="#888" />
          </g>
        );
      })()}
...
function Realistic2Deck({ color, armAngle, armLen, vinylRadius }) {
  ...
  return (
    <svg viewBox="0 0 560 560" className="deckSvg">
      ...
      <rect x="452" y="118" width="74" height="344" rx="28" fill="rgba(255,255,255,0.18)" stroke="rgba(255,255,255,0.28)" />
      ...
      <circle cx="489" cy="214" r="21" fill="url(#r2-knob)" stroke="rgba(0,0,0,0.2)" />
      <circle cx="489" cy="286" r="21" fill="url(#r2-knob)" stroke="rgba(0,0,0,0.2)" />
      <line x1="489" y1="200" x2="489" y2="191" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" />
      <line x1="489" y1="272" x2="489" y2="263" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" />

      <rect x="470" y="338" width="38" height="82" rx="19" fill="rgba(0,0,0,0.24)" />
      <rect x="464" y="370" width="50" height="22" rx="11" fill="rgba(255,255,255,0.72)" />

      <circle cx={pivotX} cy={pivotY} r="25" fill="rgba(255,255,255,0.22)" stroke="rgba(255,255,255,0.32)" filter="url(#r2-soft)" />
      <circle cx={pivotX} cy={pivotY} r="12" fill="url(#r2-knob)" />

      {(() => {
        const nx = pivotX - armLen;
        return (
          <g transform={`rotate(${armAngle} ${pivotX} ${pivotY})`}>
            <rect x={nx} y={pivotY - 4.5} width={armLen} height="9" rx="4.5" fill="url(#r2-arm)" />
            <rect x={nx + 2} y={pivotY - 4} width={armLen - 4} height="3.5" rx="1.5" fill="#fff" opacity="0.35" />
            <path d={`M${nx - 18},${pivotY - 10} L${nx + 9},${pivotY - 6} L${nx + 5},${pivotY + 11} L${nx - 18},${pivotY + 8} Z`} fill="#cfcfcf" stroke="#888" />
            <rect x={nx - 12} y={pivotY + 2} width="16" height="10" rx="2" fill="#303030" />
            <line x1={nx - 4} y1={pivotY + 12} x2={nx - 4} y2={pivotY + 2} stroke="#222" strokeWidth="2" strokeLinecap="round" />
            <ellipse cx={pivotX + 18} cy={pivotY} rx="14" ry="9" fill="#a0a0a0" stroke="#777" />
          </g>
        );
      })()}
...
// REALISTIC3 - fixed without invalid full-screen white overlay
...
// This corrected definition overrides the earlier Realistic3Deck from Teil 3.
// It keeps the chassis transparent around the platter hole, so the vinyl is visible.
function Realistic3Deck({ armAngle, armLen, vinylRadius }) {
  const cx = 265;
  const cy = 285;
  const vr = vinylRadius + 8;
  ...
  return (
    <svg viewBox="0 0 760 560" className="deckSvg">
      <defs>
        <filter id="r3-shadow-fixed">
          <feDropShadow dx="0" dy="6" stdDeviation="12" floodOpacity="0.45" />
        </filter>
        <filter id="r3-soft-fixed">
          <feDropShadow dx="0" dy="2" stdDeviation="3" floodOpacity="0.3" />
        </filter>
        <linearGradient id="r3-plinth-fixed" x1="0" y1="0" x2="0.3" y2="1">
          <stop offset="0%" stopColor="#e8e0d0" />
          <stop offset="40%" stopColor="#d8cdb8" />
          <stop offset="100%" stopColor="#c8bda8" />
        </linearGradient>
        <linearGradient id="r3-panel-fixed" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#2a2520" />
          <stop offset="50%" stopColor="#1e1a16" />
          <stop offset="100%" stopColor="#161210" />
        </linearGradient>
        <linearGradient id="r3-arm-fixed" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#e8e8e8" />
          <stop offset="30%" stopColor="#c8c8c8" />
          <stop offset="70%" stopColor="#a0a0a0" />
          <stop offset="100%" stopColor="#787878" />
        </linearGradient>
        <linearGradient id="r3-arm-shine-fixed" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="rgba(255,255,255,0.55)" />
          <stop offset="100%" stopColor="rgba(255,255,255,0)" />
        </linearGradient>
        <radialGradient id="r3-pivot-fixed" cx="35%" cy="30%" r="65%">
          <stop offset="0%" stopColor="#e0e0e0" />
          <stop offset="50%" stopColor="#b0b0b0" />
          <stop offset="100%" stopColor="#606060" />
        </radialGradient>
      ...
      <path d={`M8,8 L484,8 L484,552 L8,552 Z ${hole}`} fill="url(#r3-plinth-fixed)" fillRule="evenodd" filter="url(#r3-shadow-fixed)" />
      <rect x="9" y="9" width="475" height="542" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
      ...
      <rect x="486" y="8" width="4" height="544" rx="1" fill="#0e0c0a" />
      <rect x="492" y="8" width="260" height="544" rx="6" fill="url(#r3-panel-fixed)" />
      <rect x="493" y="9" width="258" height="542" rx="5" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
      ...
      <rect x="328" y="460" width="72" height="52" rx="5" fill="rgba(0,0,0,0.18)" stroke="rgba(0,0,0,0.35)" strokeWidth="1" filter="url(#r3-soft-fixed)" />
      <text x="350" y="474" fill="#666" fontSize="7" fontFamily="monospace" letterSpacing="1">LIFT</text>
      <rect x="336" y="478" width="56" height="10" rx="3" fill="#1e1e1e" stroke="#555" strokeWidth="0.8" />
      <rect x="336" y="492" width="56" height="14" rx="3" fill="#2a2a2a" stroke="#555" strokeWidth="0.8" />
      <rect x="352" y="478" width="24" height="24" rx="4" fill="#c0c0c0" filter="url(#r3-soft-fixed)" />

      <circle cx={pivotX} cy={pivotY} r="26" fill="rgba(200,190,170,0.95)" stroke="rgba(150,140,120,0.8)" strokeWidth="1.5" filter="url(#r3-soft-fixed)" />
      <circle cx={pivotX} cy={pivotY} r="20" fill="url(#r3-pivot-fixed)" />
      <circle cx={pivotX} cy={pivotY} r="6" fill="#c8c8c8" stroke="#888" strokeWidth="1" />

      <g transform={`rotate(${armAngle} ${pivotX} ${pivotY})`}>
        <rect x={nx} y={pivotY - 5} width={armLen} height="10" rx="5" fill="url(#r3-arm-fixed)" />
        <rect x={nx + 4} y={pivotY - 4} width={armLen - 8} height="4" rx="2" fill="url(#r3-arm-shine-fixed)" />
        <rect x={nx - 18} y={pivotY - 10} width="28" height="22" rx="3" fill="#c0bdb8" stroke="#888" strokeWidth="0.8" filter="url(#r3-soft-fixed)" />
        <rect x={nx - 14} y={pivotY + 1} width="18" height="12" rx="2" fill="#3a3a3a" stroke="#555" strokeWidth="0.6" />
        <line x1={nx - 8} y1={pivotY + 13} x2={nx - 8} y2={pivotY + 6} stroke="#666" strokeWidth="1.5" strokeLinecap="round" />
        <circle cx={nx - 8} cy={pivotY + 14} r="1.8" fill="#222" />
        <ellipse cx={pivotX + 16} cy={pivotY} rx="14" ry="11" fill="#b0b0b0" stroke="#888" strokeWidth="0.8" />
  ...
  if (style === "mist") {
    ...
    for (let i = 0; i < 46; i++) {
      ...
      items.push(
        <path
          key={i}
          d={`M ${cx + Math.cos(a) * ir} ${cy + Math.sin(a) * ir} C ${cx + Math.cos(a + 0.18) * 118} ${cy + Math.sin(a + 0.18) * 118}, ${cx + Math.cos(a - 0.1) * 150} ${cy + Math.sin(a - 0.1) * 150}, ${cx + Math.cos(a) * or} ${cy + Math.sin(a) * or}`}
          stroke={color}
...
// App body stays the same structure as before, with fixed geometry.
export default function App() {
  ...
  useEffect(() => {
    async function loadAll() {
      ...
      for (const name of names) {
        ...
        if (data) meta[name] = { ...data, deckStyle: normalizeDeckStyle(data.deckStyle), tracks: (data.tracks || []).map(({ url, ...r }) => r) };
      }
  ...
  const geometry = {
    ...
    dark: { cx: 240, cy: 290, w: 560, arm: isSingle ? 188 : 176, start: isSingle ? -20 : -11, end: isSingle ? -36 : -29 },
    chrome: { cx: 240, cy: 290, w: 560, arm: isSingle ? 188 : 176, start: isSingle ? -20 : -11, end: isSingle ? -36 : -29 },
    wood: { cx: 240, cy: 290, w: 560, arm: isSingle ? 188 : 176, start: isSingle ? -20 : -11, end: isSingle ? -36 : -29 },
    realistic1: { cx: 240, cy: 290, w: 560, arm: isSingle ? 188 : 176, start: isSingle ? -20 : -11, end: isSingle ? -36 : -29 },
    realistic2: { cx: 240, cy: 290, w: 560, arm: isSingle ? 188 : 176, start: isSingle ? -20 : -11, end: isSingle ? -36 : -29 },
    ...
    <div style={S.app}>
      ...
      <div style={S.stage}>
        <div style={{ position: "relative", width: geometry.w, height: 560 }}>
          ...
          <TurntableDeck style={deckStyleNorm} color={deckColor} armAngle={armAngle} armLen={geometry.arm} vinylRadius={vinylRadius} />
        </div>
      </div>
